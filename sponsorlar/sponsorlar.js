import { isAdmin, onAdminChange, getCurrentAdminUser } from "../src/admin.js";
...
async function loadSponsors() {
  try {
    const snap = await getDocs(query(SPONSORS_COL, orderBy("createdAt", "asc")));
    return snap.docs.map(d => ({ id: d.id, ...d.data() }));
  } catch (e) { console.error("[loadSponsors]", e); return []; }
}
async function addSponsorToDb(data)       { return (await addDoc(SPONSORS_COL, data)).id; }
async function deleteSponsorFromDb(id)    { await deleteDoc(doc(db, "sponsors", id)); }
async function updateSponsorInDb(id, data){ await updateDoc(doc(db, "sponsors", id), data); }
...
function renderSponsors() {
  ...
  if (!sponsors.length) {
    grid.innerHTML = `<div class="empty-state" style="grid-column:1/-1"><div class="icon">🏅</div><p>Henüz sponsor eklenmemiş.</p></div>`;
    return;
  }
  grid.innerHTML = sorted.map(sp => `
    <div class="sponsor-card">
      ${sp.image ? `<img class="sponsor-avatar" src="${esc(sp.image)}" alt="${esc(sp.name)}" onerror="this.style.display='none';this.nextElementSibling.style.display='flex'">` : ""}
  ...
  area.innerHTML = `
    <div class="admin-section">
      <div class="admin-header">
        ...
        <span style="font-size:13px;color:#666;">${esc(user?.email || "")}</span>
      </div>
...
async function addSponsor() {
  ...
  try {
    ...
    renderSponsors(); renderAdminArea(); clearForm();
...
async function deleteSponsor(id) {
  ...
  try {
    ...
    renderSponsors(); renderAdminArea();
...
async function saveEdit() {
  ...
  try {
    await updateSponsorInDb(editingId, { name: sp.name, amount: sp.amount, image: sp.image, note: sp.note });
    closeModal(); renderSponsors(); renderAdminArea();
