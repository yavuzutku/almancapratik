...
function geminiUrl(k) {
  return `https://generativelanguage.googleapis.com/v1beta/models/${k}:generateContent`;
}
...
const MODES = {
  duzelt: {
    label: "Cümle Düzelt", icon: "✏️",
    placeholder: "Almanca cümlenizi yazın, hataları düzeltelim…",
    systemPrompt: t => `Sen uzman bir Almanca dil öğretmenisin. Aşağıdaki Almanca cümleyi düzelt. Eğer hata varsa: 1) Düzeltilmiş hali yaz, 2) Hatayı kısa Türkçe açıkla. Hata yoksa "✓ Cümle doğru" de ve kısa not ekle.\n\nCümle: ${t}`,
    quickPrompts: ["Ich gehe gestern zum Markt.","Er hat viele Bücher gelest.","Die Kinder spielen in den Garten.","Ich bin 20 Jahre jung."],
  },
  cevir: {
    label: "Türkçe → Almanca", icon: "🔄",
    placeholder: "Türkçe metni yazın, Almancaya çevirelim…",
    systemPrompt: t => `Türkçeyi Almancaya doğal ve akıcı biçimde çevir. Sadece çeviriyi ver, açıklama ekleme. Birden fazla cümle varsa hepsini çevir.\n\nMetin: ${t}`,
    quickPrompts: ["Merhaba, nasılsınız?","Bugün hava çok güzel.","Sizi tanımaktan büyük memnuniyet duydum.","Bu kitabı okumak istiyorum."],
  },
  gramer: {
    label: "Gramer Sor", icon: "📖",
    placeholder: "Almanca gramer sorunuzu yazın…",
    systemPrompt: t => `Sen deneyimli bir Almanca öğretmenisin. Soruyu Türkçe, anlaşılır ve örneklerle açıkla. Örnekleri kalın yaz (**örnek**). Net ve öğretici ol.\n\nSoru: ${t}`,
    quickPrompts: ["Akkusativ ve Dativ ne zaman kullanılır?","Perfekt ve Präteritum farkı nedir?","Trennbare Verben nasıl kullanılır?","Konjunktiv II nedir?"],
  },
  kelime: {
    label: "Kelime Analizi", icon: "🔍",
    placeholder: "Analiz edilecek Almanca kelimeyi yazın…",
    systemPrompt: t => `"${t}" kelimesini analiz et:\n- Artikel (isimse): der/die/das\n- Kelime türü\n- Türkçe anlam(lar)\n- Çoğul formu (isimse)\n- 3 örnek cümle (Almanca + Türkçe çeviri)\n- Varsa yaygın deyimler\n\nAçıklamaları Türkçe yap. Örnek cümleleri numaralandır.`,
    quickPrompts: ["Hund","gehen","schön","Freundschaft","weil","obwohl"],
  },
  cumle: {
    label: "Cümle Üret", icon: "✨",
    placeholder: "Kelime veya konu yazın, cümleler üretelim…",
    systemPrompt: t => `"${t}" konusunda veya kelimesiyle 10 Almanca örnek cümle yaz. Her cümlenin altına Türkçe çevirisini ekle. Cümleleri 1'den 10'a kadar numaralandır. Kolay ve orta seviye karışık olsun.`,
    quickPrompts: ["sein (fiili)","Wetter (hava)","Familie","Arbeit","essen","gehen"],
  },
  serbest: {
    label: "Serbest Sohbet", icon: "💬",
    placeholder: "Almanca ile ilgili her şeyi sorabilirsiniz…",
    systemPrompt: t => `Sen Almanca öğrenmeye yardımcı olan, Türkçe açıklama yapan bir dil asistanısın. Soruyu eksiksiz ve açıklayıcı yanıtla.\n\n${t}`,
    quickPrompts: ["A1 için en önemli 10 fiil","Almanca öğrenme tavsiyeleri","Zaman zarfları listesi","B1 sınavı için ipuçları"],
  },
};
...
/* ══════════════════════════════════════════════
   AUTH GUARD — merkezi admin modülü
══════════════════════════════════════════════ */
requireAdmin({
  showDeniedId:  "accessDenied",
  showContentId: "aiContent",
});

/* aiContent görününce init et */
const _obs = new MutationObserver(() => {
  const el = document.getElementById("aiContent");
  if (el && el.style.display !== "none") { _obs.disconnect(); init(); }
});
...
function setMode(key) {
  ...
  document.querySelectorAll(".mode-tab").forEach(b => b.classList.toggle("active", b.dataset.mode === key));
...
function bindEvents() {
  ...
  document.getElementById("promptInput")?.addEventListener("input", e => updateCharCount(e.target.value.length));
  ...
  document.getElementById("retryBtn")?.addEventListener("click", () => { if (lastPrompt) sendPrompt(lastPrompt); });
}
...
async function sendPrompt(retryText) {
  ...
  const text   = typeof retryText === "string" ? retryText : document.getElementById("promptInput")?.value.trim();
  if (!apiKey) { showError("Önce Gemini API key'inizi girin ve Kaydet butonuna basın.", false); return; }
  ...
  try {
    ...
    if (!res.ok) {
      ...
      if (res.status === 503) showError(`Gemini sunucusu şu an meşgul (503). Birkaç saniye bekleyip tekrar deneyin.`, true);
      else if (res.status === 400) showError(`API hatası: ${msg} — API key'inizi kontrol edin.`, false);
      else showError(`Sunucu hatası (${res.status}): ${msg}`, true);
      return;
    }
    const data   = await res.json();
    const answer = data?.candidates?.[0]?.content?.parts?.[0]?.text;
    if (!answer) { showError(`Yanıt alınamadı. Neden: ${data?.candidates?.[0]?.finishReason || "bilinmiyor"}`, true); return; }
...
function renderMarkdown(raw) {
  ...
  let html = "", inCode = false, codeBuffer = "";
  for (const line of lines) {
    ...
    if (/^#{1,3}\s/.test(line)) { html += `<span class="r-h">${applyInline(esc(line.replace(/^#{1,3}\s*/, "")))}</span>`; continue; }
    if (/^[\*\-]\s/.test(line.trim())) { html += `<div class="r-li"><span class="r-li-bullet">›</span><span>${applyInline(esc(line.trim().replace(/^[\*\-]\s/, "")))}</span></div>`; continue; }
    const nm = line.trim().match(/^(\d+)\.\s(.*)/);
    if (nm) { html += `<div class="r-li"><span class="r-li-bullet">${esc(nm[1])}.</span><span>${applyInline(esc(nm[2]))}</span></div>`; continue; }
    if (/^---+$/.test(line.trim())) { html += `<hr class="r-separator">`; continue; }
...
async function copyResponse() {
  ...
  try {
    ...
    setTimeout(() => { btn.textContent = "Kopyala"; btn.classList.remove("success"); }, 2000);
  } catch { /* pass */ }
}
