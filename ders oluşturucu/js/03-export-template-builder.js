...
'<button type="button" class="lesson-complete-btn" id="btnToggleComplete" data-lesson-slug="' + slugPart + '" onclick="toggleLessonComplete()">' +
'<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><polyline points="20 6 9 17 4 12"/></svg>' +
'<span id="btnToggleCompleteText">Tamamlandı olarak işaretle</span></button>',
"</div>",
...
"  handleAutoComplete(scrolled);",
"});",

"/* 1b. Ders \"Tamamlandı\" takibi (localStorage, /dersler/ ana listesiyle aynı",
"   'ap_completed_lessons' anahtarını kullanır — böylece bu statik ders",
"   sayfasında verilen işaret, /dersler/ listesindeki seviye akordeonuna da yansır). */",
"var COMPLETED_KEY = 'ap_completed_lessons';",
"var __autoCompleteFired = false;",
...
"  var slug = btn ? btn.getAttribute('data-lesson-slug') : '';",
"  return slug ? getCompletedSet().has(slug) : false;",
"}",
"function setCurrentLessonCompleted(completed) {",
"  var btn = document.getElementById('btnToggleComplete');",
"  var slug = btn ? btn.getAttribute('data-lesson-slug') : '';",
"  if (!slug) return;",
"  var set = getCompletedSet();",
"  if (completed) set.add(slug); else set.delete(slug);",
"  try { localStorage.setItem(COMPLETED_KEY, JSON.stringify(Array.from(set))); } catch (e) {}",
"  updateCompleteButtonUI();",
"}",
"function updateCompleteButtonUI() {",
"  var btn = document.getElementById('btnToggleComplete');",
"  var txt = document.getElementById('btnToggleCompleteText');",
"  if (!btn) return;",
"  var done = isCurrentLessonCompleted();",
"  btn.classList.toggle('is-done', done);",
"  if (txt) txt.textContent = done ? 'Tamamlandı ✓' : 'Tamamlandı olarak işaretle';",
"}",
"function toggleLessonComplete() {",
"  setCurrentLessonCompleted(!isCurrentLessonCompleted());",
"}",
"function handleAutoComplete(scrolledPct) {",
"  if (__autoCompleteFired || isCurrentLessonCompleted()) return;",
"  if (scrolledPct >= 92) { __autoCompleteFired = true; setCurrentLessonCompleted(true); }",
"}",
"updateCompleteButtonUI();",
